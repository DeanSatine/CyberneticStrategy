...
public class TestGameManager : MonoBehaviour
{
    ...
    public void RegisterUnit(UnitAI unit, bool isPlayer)
    {
        ...
        if (isPlayer && TestUIManager.Instance != null)
        {
    ...
    public void UnregisterUnit(UnitAI unit)
    {
        ...
        // ✅ Update fight button visibility when units change
        if (wasPlayerUnit && TestUIManager.Instance != null)
        {
            TestUIManager.Instance.UpdateFightButtonVisibility();
        }
    }
        ...
        for (int starLevel = 1; starLevel <= 2; starLevel++)
        {
            ...
            foreach (var group in unitGroups)
            {
                ...
                while (availableUnits.Count >= 3)
                {
                    ...
                    {
                        Vector3 vfxPosition = upgradedUnit.transform.position;
                        vfxPosition.y += 1f; 

                        var vfx = Instantiate(starUpVFXPrefab, vfxPosition, Quaternion.identity);
                        Destroy(vfx, 2f);
                        Debug.Log($"✨ Spawned star-up VFX at {vfxPosition}");
                    }
                    else
                    {
                        Debug.LogWarning("⚠️ starUpVFXPrefab is null - cannot spawn VFX");
                    }
                    ...
                    if (starUpSound != null)
                    {
                        ...
                        if (audioSource == null)
                        {
                            ...
                            Debug.Log("➕ Added AudioSource to TestGameManager");
                        }
                        audioSource.PlayOneShot(starUpSound);
                        Debug.Log($"🔊 Playing star-up sound");
                    }
                    else
                    {
                        Debug.LogWarning("⚠️ starUpSound is null - cannot play sound");
                    }
