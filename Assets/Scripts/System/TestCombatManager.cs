...
{
    public static TestCombatManager Instance;
    [System.Serializable]
    ...
    private TestStageManager.GamePhase currentPhase => TestStageManager.Instance?.currentPhase ?? TestStageManager.GamePhase.Prep;
    ...
    public void StartCombat()
    {
        ...
        List<UnitAI> playerUnits = TestGameManager.Instance.GetPlayerUnits();
        ...
        List<UnitAI> enemyUnits = TestGameManager.Instance.GetEnemyUnits();
    ...
    private void SavePlayerPositions()
    {
        ...
        foreach (var unit in TestGameManager.Instance.GetPlayerUnits())
        {
            if (unit != null && unit.currentState != UnitAI.UnitState.Bench)
            {
    ...
    public void RestorePlayerUnitsFromSnapshots()
    {
        ...
        var playerUnitsCopy = new List<UnitAI>(TestGameManager.Instance.GetPlayerUnits());
    ...
    private void RestoreExistingUnit(UnitAI unit, UnitSnapshot snapshot)
    {
        ...
        if (!TestGameManager.Instance.GetPlayerUnits().Contains(unit))
        {
            TestGameManager.Instance.RegisterUnit(unit, unit.team == Team.Player);
        }
    ...
    private void TrackCombatDeath(UnitAI deadUnit)
    {
        ...
        if (deadUnit.team == Team.Player && currentPhase == TestStageManager.GamePhase.Combat)
        {
    ...
    private void CheckForRoundEnd()
    {
        ...
        if (TestStageManager.Instance?.currentPhase != TestStageManager.GamePhase.Combat)
        {
    ...
    private IEnumerator DelayedRoundTransition(bool playerWon)
    {
        ...
        if (TestStageManager.Instance != null)
        {
            TestStageManager.Instance.OnCombatEnd(playerWon);
        }
        else
        {
