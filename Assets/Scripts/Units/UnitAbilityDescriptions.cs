...
public static class UnitAbilityDescriptions
{
    ...
    private static string GetBackSlashDescription(UnitAI unit, float ad, int star)
    {
        ...
        float slashAPRatio = ability.slashAPRatio[starIndex];
        float slamAPRatio = ability.slamAPRatio[starIndex];

        float totalSlashDmg = slashDmg + (unit.abilityPower * slashAPRatio);
        float totalSlamDmg = slamDmg + (unit.abilityPower * slamAPRatio);

        return $"Slash 1 hex in front of BackSlash, dealing {totalSlashDmg:F0} <color=#00BFFF>magic damage</color> ({slashDmg} + {slashAPRatio * 100:F0}% AP).\n\n" +
               $"Every 3rd cast, slam down dealing {totalSlamDmg:F0} <color=#00BFFF>magic damage</color> ({slamDmg} + {slamAPRatio * 100:F0}% AP) in a {ability.slamRadius / 3f:F0} hex radius.";
    }
    ...
    private static string GetCoreweaverDescription(UnitAI unit, float ad, int star)
    {
        ...
        return $"Unleash a storm of meteors and lightning in a large area around the current target for <color=#FFD700>{duration}</color> seconds.\n" +
    ...
    private static string GetHaymakerDescription(UnitAI unit, float ad, int star)
    {
        ...
        return $"Dash to the center clump of enemies and unleash a fury of slashes within 3 hexes that each do {slashDmg + (ad * 1.2f)} <color=#FF6600>physical damage</color> for 3 seconds. While slashing take {damageReductionPercent}% reduced damage. Then dash back to original position.\n\n" +
    ...
    private static string GetKuromushadoDescription(UnitAI unit, float ad, int star)
    {
        ...
        int starIndex = Mathf.Clamp(star - 1, 0, ability.coneSizePerStar.Length - 1);
        int coneSize = ability.coneSizePerStar[starIndex];
        float kickBaseDmg = ability.jumpKickDamage[starIndex];
        float kickADRatio = ability.jumpKickADRatio[starIndex];

        float totalKickDmg = kickBaseDmg + (ad * kickADRatio);
        float coneBonusDmg = ad * ability.coneBonusADRatio;

        return $"Passive: Auto Attacks sweep in a {coneSize} hex cone, hitting all enemies within {ability.coneAngle}° of the target for base damage + {ability.coneBonusADRatio * 100:F0}% AD bonus ({ad:F0} + {coneBonusDmg:F0} = {ad + coneBonusDmg:F0} total).\n\n" +
               $"Active: Jump Kick the target, dealing {totalKickDmg:F0} <color=#FF6600>physical damage</color> ({kickBaseDmg} + {kickADRatio * 100:F0}% AD) and knocking them back {ability.knockbackDistance} hexes.";
    }
    ...
    private static string GetSightlineDescription(UnitAI unit, float ad, int star)
    {
        ...
        int starIndex = Mathf.Clamp(star - 1, 0, 2);
        float baseDPS = ability.damagePerSecond[starIndex];
        float adRatio = ability.dpsADRatio[starIndex];
        float totalDPS = baseDPS + (ad * adRatio);
        ...
        return $"Passive: Attacks grant {ability.attackSpeedPerStack * 100:F0}% stacking attack speed.\n\n" +
               $"Active: Summon a portal that shoots piercing lasers dealing {totalDPS:F0} <color=#FF6600>physical damage</color> ({baseDPS} + {adRatio * 100:F0}% AD) " +
    ...
    private static string GetSteelGuardDescription(UnitAI unit, float ad, int star)
    {
        ...
        float baseShield = ability.shieldValue[starIndex];
        float apRatio = ability.shieldAPRatio[starIndex];

        float shieldAmount = baseShield + (unit.abilityPower * apRatio);
